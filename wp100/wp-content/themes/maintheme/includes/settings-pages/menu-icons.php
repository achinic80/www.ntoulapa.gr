    ...
    <div class="field-custom_menu_icon description-wide" style="margin: 5px 0;">
        ...
        <div class="logged-input-holder my_meta_control">
            <div class="d-flex align-items-center">
                <input type="checkbox" id="menu-header-for-<?php echo $item_id; ?>" name="menu_header[<?php echo $item_id; ?>]" value="1" <?php echo checked($menuheader, 1, false); ?> class="mr-1" />
                <label for="menu_header[<?php echo $item_id; ?>]">Is this a header?</label>
            </div>
            <br />
            <div class="d-flex align-items-center">
                <div class="image-preview mr-1 col-2" id="custom-menu-meta-for-<?php echo $item_id; ?>_preview">
                    <?php if (isset($custom_menu_icon)) { ?>
                        <img width="50" src="<?php echo $custom_menu_icon ?>" class="img-fluid" loading="lazy" style="max-height: 90px;">
                    <?php } ?>
                </div>
                <input type="text" id="custom-menu-meta-for-<?php echo $item_id; ?>" name="custom_menu_icon[<?php echo $item_id; ?>]" value="<?php echo esc_attr($custom_menu_icon); ?>" class="mr-1 w-100" />
...
function custom_menu_nav_update($menu_id, $menu_item_db_id)
{
    ...
    if (isset($_POST['custom_menu_icon'][$menu_item_db_id])) {
        ...
        $search  = home_url();
        $replace = '';
        $iconrelative = str_replace($search, $replace, $_POST['custom_menu_icon'][$menu_item_db_id]);

        $sanitized_data = sanitize_text_field($iconrelative);
